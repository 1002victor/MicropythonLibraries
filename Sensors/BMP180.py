...
class BMP180(object):
	def __init__(self,iicbus,address=BMP180_IICADDR,mode=BMP180_STANDARD):
		self._mode = mode
	...
	def read_temperature(self,offset = 0):
		UT = self.read_raw_temp()
		X1 = ((UT - self.cal_AC6) * self.cal_AC5) >> 15
		X2 = (self.cal_MC << 11) // (X1 + self.cal_MD)
		B5 = X1 + X2
		temp = ((B5 + 8) >> 4) / 10.0 + offset
		return temp
	def read_pressure(self,offset = 0):
...
def main():	
	...
	bmp=BMP180(iic)
	p = bmp.read_sealevel_pressure(0.0)
	temp = bmp.read_temperature()
	pressure = bmp.read_pressure()
	altitude = bmp.read_altitude(p)
